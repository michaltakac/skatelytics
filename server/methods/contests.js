Meteor.methods({
  '/contests/add': function (params) {
    Contests.insert(params);
  },
  'updateScore': function(contestId, skaterId, newScore, oldScore) {
    var score = parseFloat((newScore + oldScore) / 2).toFixed(2);
    if (!isNaN(score)) { 
      Contests.update({"_id": contestId, "registeredSkaters._id": skaterId}, {
        $set: { 'registeredSkaters.$.score': score }
      });
    } else {
      alert("error! its NaN")
    }
